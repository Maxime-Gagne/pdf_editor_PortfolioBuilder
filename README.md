

# PortfolioBuilder

A local, privacy-first PDF editor and merger built with Python and FastAPI.
This tool allows users to reorganize, merge, and edit PDF files without uploading data to the cloud.


PortfolioBuilder runs 100% locally on your computer. Your files never leave your machine.


## Installation (For Developers)

1. Clone the repository
2. Install dependencies:
   ```bash
   pip install -r requirements.txt

========================================================================
**✨ KEY FEATURES**
========================================================================

* 🔒 100% Private: No cloud uploads. Everything stays on your disk.
* 💸 Lifetime Deal: No subscriptions. You own this version forever.
* 📑 Drag & Drop Sorting: Easily reorganize pages from multiple files.
* 🛠️ Quick Edits: Add text, cover mistakes, insert images or signatures.
* ⚡ Lightweight: No heavy installation required.

========================================================================
**⚖️ LICENSE & SOURCE CODE (Open Source Compliance)**
========================================================================

PortfolioBuilder is Free Software.
It is built upon the powerful PyMuPDF library.

In compliance with the GNU Affero General Public License v3 (AGPLv3), the full source code for this application is available for free to anyone who wishes to inspect, modify, or compile it themselves.

🔗 Source Code Repository: https://github.com/Maxime-Gagne/pdf_editor_PortfolioBuilder
📄 License: GNU AGPL v3 (See LICENSE.txt included in this folder)

Copyright (C) 2025 Maxime Gagné

========================================================================
**CREDITS**
========================================================================
Powered by:
- Python
- PyMuPDF (Artifex Software, Inc.)
- Fabric.js
- PDF.js


